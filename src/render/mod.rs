...
/// How vertex labels are formatted on the tetrahedron.
pub enum TetraLabelMode {
    /// Show compass-style labels (RUFK in camera frame, XYZW in world frame)
    Compass(CompassFrameMode),
    /// Show raw axis labels (XYZW)
    Raw,
    /// Hide vertex labels entirely
    Hidden,
}

/// Visual styling for tetrahedron rendering.
...
/// Legacy entry point for compass tetrahedron rendering with a `StereoProjector`.
